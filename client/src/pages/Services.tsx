...
import { licenseFacts, licensedScopes } from '@/data/licenseScope';
import { SSG_LOGO, SSG_HERO_HELMET, SSG_LICENSE_MAIN } from '@/data/presentationContent';
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--sand)' }}>
        ...
        <div className="relative mx-auto max-w-7xl">
          <motion.div variants={up()} initial="hidden" animate="visible" className="max-w-3xl">
            ...
            <h1 className="text-5xl font-bold mb-5 sm:text-6xl md:text-7xl" style={{ color: 'var(--sand)' }}>
              {t.services.hero.split(' ').slice(0, 2).join(' ')}<br />
              {t.services.hero.split(' ').slice(2).join(' ')}
            </h1>
      ...
      <section className="px-4 py-16 md:px-8" style={{ backgroundColor: 'var(--sand-dark)' }}>
        <div className="mx-auto max-w-7xl">
          <div className="grid grid-cols-1 gap-8 items-start xl:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <motion.div variants={up()} initial="hidden" whileInView="visible" viewport={{ once: true }}
              ...
              <div className="bg-white grid grid-cols-1 sm:grid-cols-2">
                {licenseFactsData.map((fact) => (
                  <div key={fact.label} className={`p-5 border-b ${fact.wide ? 'sm:col-span-2' : ''}`}
            ...
            <motion.div variants={up(1)} initial="hidden" whileInView="visible" viewport={{ once: true }}
              className="space-y-3">
              <div className="font-mono text-xs mb-3 font-semibold" style={{ color: 'var(--ember)' }}>
                {t.services.licenseDocs}
              </div>
              <div className="rounded-xl overflow-hidden shadow-md group aspect-[3/4] bg-white"
                style={{ border: '1px solid rgba(13,13,13,0.1)' }}>
